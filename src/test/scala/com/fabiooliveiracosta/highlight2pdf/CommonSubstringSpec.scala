...
class CommonSubstringSpec extends UnitSpec {
	...
	"The longest common substring" should "be found if fully matched" in{
		...
		val comparison=CommonSubstring.findSegmentPosition(needle,haystack)
  	...
  	"The longest common substring" should "be found if partialy matched" in{
  		...
  		val comparison=CommonSubstring.findSegmentPosition(baseNeedle+"Some stuff that should never be found",haystack)
